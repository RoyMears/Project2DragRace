...
void SysTick_Start(uint32_t period) {	
	NVIC_ST_RELOAD_R = period -1;		// reload value 800000-1;	period is the amount of cycles of half seconds we want to perform
  NVIC_ST_CURRENT_R = 0;      			      // any write to current clears it
	NVIC_ST_CTRL_R |= NVIC_ST_CTRL_CLK_SRC|NVIC_ST_CTRL_INTEN|NVIC_ST_CTRL_ENABLE;	// enable interupt SysTick with core clock and interrupts (7)
}
// Stop the Systick timer by disabling the timer
// Dependencies: NVIC_SYS_PRI3_R, NVIC_ST_CTRL_R
void SysTick_Stop(void) {
		NVIC_ST_CTRL_R = 0;       // Stop the Systick by disabling it with = 0
}
