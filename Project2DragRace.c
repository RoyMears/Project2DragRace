...
int main(void){
  ...
  while(1){
		while (!reset) {
			  ...
			  LIGHT_PORTB = (DraGrace_FSM[S].Out);// was LIGHT_PORTB = (DraGrace_FSM[S].Out & LIGHT_PORTB_MASK);
			  LIGHT_PORTC = (DraGrace_FSM[S].Out);// was LIGHT_PORTC = (DraGrace_FSM[S].Out & LIGHT_PORTC_MASK);
...
void GPIOPortx_Handler(void){
			...
			debounce();
	if (GPIO_PORTA_RIS_R & PUSHBUTTON_LEFT_MASK) { // if left button is pressed
		if (LIGHT_PORTB == 0xC0 || LIGHT_PORTB == 0x30) {// c
			S = falseStartRight; // state is false start on right
	...
	debounce();
	if (GPIO_PORTA_RIS_R & PUSHBUTTON_RIGHT_MASK) { // if right button is pressed
		if (LIGHT_PORTB == 0xC0 || LIGHT_PORTB == 0x30) { // if both red leds are on, or both green leds are on
			S = falseStartLeft; // state is false start on left
			GPIO_PORTA_ICR_R = PUSHBUTTON_RIGHT_MASK; // clear flag
		}
		else {
			GPIO_PORTA_ICR_R = PUSHBUTTON_RIGHT_MASK; // clear flag
		}
	...
	debounce();
	if (GPIO_PORTA_RIS_R & PUSHBUTTON_RIGHT_MASK) { // if both buttons are pressed
		if (LIGHT_PORTB == 0xC0 || LIGHT_PORTB == 0x30) { // if both red leds are on, or both green leds are on
			S = falseStartBoth; // state if false start on both
...
void GPIOPortE_Handler(void) {
	if(RESET & RESET_MASK){
		...
		GPIO_PORTE_IM_R 	|= RESET_MASK;															// arm interrupt on PE1 (allow interrupt to be sent)
	} else {
		GPIO_PORTE_ICR_R 	|= RESET_MASK;															// clear flag
	}
}
